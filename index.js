...
function getKeysStartingWith(dictionary, letter) {
  ...
  for (const [key, value] of dictionary)
    if (key[0] === letter) arrKeys.push(key);
  return arrKeys;
}
...
function transformDictionary(dictionary) {
  ...
  let values = [];
  let entries = [];
  for (const key of dictionary.keys()) keys.push(key);
  for (const value of dictionary.values()) values.push(value);
  for (const entrie of dictionary.entries()) entries.push(entrie);
...
function sortByValues(dictionary) {
  ...
  const arrEntried = [...dictionary.entries()];
  arrEntried.sort(([key, value], b) => b[1] - value);
  return new Map(arrEntried);
}
...
function resetDictionary(dictionary, maxSize) {
  ...
  if (dictionary.size > maxSize) dictionary.clear();
...
function convertDictionaryToSet(dictionary) {
  ...
  for (const key of dictionary.keys())
    if (typeof key === "string") newSet.add(key.toUpperCase());
    else if (typeof key === "number") newSet.add(key + 1);
  return newSet;
}
...
function convertSetToDictionary(set) {
  ...
  for (const element of set) dictionary.set(element, element.charCodeAt(0));
  return dictionary;
}
